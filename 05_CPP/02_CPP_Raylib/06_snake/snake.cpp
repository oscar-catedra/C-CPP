...
int main() {
    ...
    while (!WindowShouldClose()) {
        if (!gameOver) {
            ...
            if (timer >= delay) {
                ...
                if (snake[0].x == food.x && snake[0].y == food.y) {
                    food = { GetRandomValue(0, 39), GetRandomValue(0, 29) };
        ...
        for (const auto& seg : snake) {
            DrawRectangle(seg.x * 20, seg.y * 20, 18, 18, GREEN);
        }

        DrawRectangle(food.x * 20, food.y * 20, 18, 18, RED);
